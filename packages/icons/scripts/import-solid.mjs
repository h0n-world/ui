...
const imports = sourceItems.map((item) => {
  const preferredName = duplicateBaseNames.has(item.baseName)
    ? `${item.category}-${item.baseName}`
    : item.baseName;
  const occupiedByAnotherStyle =
    metadata.icons[preferredName] && metadata.icons[preferredName].style !== 'solid';
  return {
    ...item,
    name: occupiedByAnotherStyle ? `${preferredName}-solid` : preferredName,
  };
});
...
for (const item of imports) {
  ...
  svg = svg
    ...
    .trim();
  normalizedImports.push({ ...item, svg });
}
...
for (const item of normalizedImports) {
  if (metadata.icons[item.name]?.style === 'solid') replaced.push(item.name);
  await writeFile(resolve(svgDir, `${item.name}.svg`), `${item.svg}\n`);
...
console.log(`Imported ${normalizedImports.length} solid SVG icons.`);
console.log(`Replaced ${replaced.length} matching solid icons: ${replaced.join(', ') || 'none'}.`);
console.log(
  `Replaced ${displaced.length} ambiguous legacy solid names with category-prefixed variants: ${displaced.join(', ') || 'none'}.`,
);
